...
# In Railway deployments, environment variables are set through Railway dashboard
try:
    from dotenv import load_dotenv
    load_dotenv()
except ImportError:
    pass  # python-dotenv not installed, skip (Railway uses platform env vars)

# Handle base64-encoded cookies if provided (for Railway environment variables)
...
def launch(args):
    ...
    with gr.Blocks(title="DVD Video Q&A Demo", css=custom_css, theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown(
            ...
            Deployed on <a href="https://railway.app" style="color: #6366f1;">Railway</a>
            </p>
            """
        )
    ...
    railway_port = int(os.environ.get("PORT", "7860"))
    demo.launch(server_name="0.0.0.0", server_port=railway_port, share=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--share", action="store_true", help="Gradio share flag (ignored on Railway)")
